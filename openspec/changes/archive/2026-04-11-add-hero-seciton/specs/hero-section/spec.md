...
Hero Section SHALL 全屏显示，居中展示用户的姓名、职业头衔和一句话简介。

#### Scenario: 正常显示个人信息
- **WHEN** 用户访问首页
- **THEN** 页面显示全屏 Hero Section
- **AND** 居中显示姓名、职业头衔、简介文字
- **AND** 显示 CTA 按钮"查看项目"
...
- **GIVEN** 视口宽度小于 768px
- **WHEN** 页面加载
- **THEN** 文字大小适配移动端（姓名 text-4xl，其他 text-lg）
- **AND** 内容水平内边距为 px-6

#### Scenario: 响应式布局 - 桌面端
- **GIVEN** 视口宽度大于等于 768px
- **WHEN** 页面加载
- **THEN** 文字大小适配桌面端（姓名 text-6xl，其他 text-xl/2xl）
- **AND** 内容最大宽度限制为 max-w-4xl
